...
import 'package:enma/pages/EventOrganizer/EditEvent.dart';
...
class History extends StatefulWidget{
  ...
  History({this.uid});
  @override
  State<StatefulWidget> createState() => new _HistoryState();

}

class _HistoryState extends State<History>{
  DateTime now = new DateTime.now().subtract(new Duration(days: 1));
                  ...
                  children: <Widget>[
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: <Widget>[
                          Padding(
                            padding: const EdgeInsets.symmetric(horizontal: 32.0),
                            child: Text('Event History', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white, fontSize: 30.0),),
                          ),
                        ],
                      ),
                    ),
                    Container(
                      child: Column(
                        ...
                        children: <Widget>[
                          StreamBuilder(
                            stream: Firestore.instance.collection("Registered Event").where("User Id", isEqualTo: widget.uid).where('Date', isLessThan: DateTime.now()).snapshots(),
                            builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot){
                              if(!snapshot.hasData){
                                return new Container(
                                  child: Center(
                                    //child: Loading1(),
                                    child: Center(child: Text("Sorry, there is no existence data history")),
                                  ),
                                );
                              }
                              return Container(
                                  height: 500.0,
                                  child: new EventHistoryList(document: snapshot.data.documents, uid: widget.uid)
                              );
                            },
                          ),
                        ],
...
class EventHistoryList extends StatelessWidget {
  EventHistoryList({this.document, this.uid});
  final String uid;
